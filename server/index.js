...
async function initializeSheetsService() {
  try {
    ...
    if (!credentialsJsonFile) {
      throw new Error('Missing GOOGLE_CREDENTIALS_JSON_FILE environment variable. Please configure the credentials JSON file path in the extension settings.');
    }

    // Check if the credentials file path contains unresolved placeholders
    if (credentialsJsonFile.includes('${user_config.')) {
      throw new Error('Environment variable interpolation failed. Credentials file path contains unresolved placeholders. Please check your extension configuration in Claude Desktop settings.');
    }
...
server.registerTool('get_spreadsheet_summary', {
  description: 'Get a summary of a Google spreadsheet including its name, number of sheets, and the names of those sheets. Returns metadata only, no actual data.',
  inputSchema: {
    url: z.string().describe('The complete Google Sheets URL (e.g., "https://docs.google.com/spreadsheets/d/1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms/edit")'),
  },
}, async ({ url }) => {
  try {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `**Google Sheet Summary: ${summaryData.title}**\n\n` +
                ...
                `**Available Sheets:**\n` +
                summaryData.sheetNames.map(sheet => 
...
server.registerTool('get_spreadsheet_sheet_data', {
  description: 'Get the actual data from a specific sheet within a Google spreadsheet. Returns complete sheet data including all cell values.',
  inputSchema: {
    url: z.string().describe('The complete Google Sheets URL (e.g., "https://docs.google.com/spreadsheets/d/1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms/edit")'),
    sheet_name: z.string().describe('The name of the specific sheet to retrieve data from (e.g., "Sheet1", "Data", "Sales Q1")'),
  },
  ...
  try {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `**Google Sheet Data: ${sheetData.spreadsheetTitle} - ${sheetData.sheet.title}**\n\n` +
